import { Clock, Tag, Zap, Heart, Recycle } from "lucide-react";
...
const CONFIG = {
  ...
  compost: { label: "Compost", icon: Recycle, color: "text-compost", bg: "bg-compost-light" },
};

export default function StatusOverview({ batches }) {
  const active = batches.filter((b) => !["claimed", "completed"].includes(b.status));

  const groups = ["hold", "markdown", "fast_track", "donate", "compost"].map((key) => {
