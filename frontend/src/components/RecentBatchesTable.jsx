...
export default function RecentBatchesTable({ batches, onSelect }) {
  const recent = batches.slice(0, 5);

  return (
    <div className="bg-panel border border-border rounded-xl p-5 shadow-card">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-ink">Recent Batches</h2>
        <span className="text-brand text-sm font-medium cursor-default">View All</span>
      </div>

      {recent.length === 0 ? (
        <p className="text-muted text-sm py-6 text-center">No batches logged yet.</p>
      ) : (
          ...
          <tbody>
            {recent.map((b) => (
              <tr key={b.id} className="border-b border-border last:border-0">
                ...
                <td className="py-3 text-muted">{b.quantity_kg} kg</td>
                <td className="py-3">
                  <ActionBadge action={b.recommended_action} discountPct={b.discount_pct} small />
                </td>
                <td className="py-3 text-muted">
                  {b.remaining_shelf_life_days <= 0 ? "Expired" : `${b.remaining_shelf_life_days}d left`}
                </td>
