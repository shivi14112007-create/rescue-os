...
export default function App() {
  ...
  useEffect(() => {
    refresh();
    // Poll periodically so the impact dashboard updates live during a demo -
    // e.g. right after a claim happens on another screen/device.
  ...
  function handleComplete(updatedBatch) {
    setBatches((prev) => prev.map((b) => (b.id === updatedBatch.id ? updatedBatch : b)));
    setSelectedBatch((prev) => (prev && prev.id === updatedBatch.id ? updatedBatch : prev));
    getImpact().then(setImpact).catch(() => {});
  }
    ...
    <div className="flex min-h-screen bg-canvas">
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="p-8">
          ...
          {page === "batches" && (
            <MyBatches batches={batches} onSelect={setSelectedBatch} onComplete={handleComplete} />
          )}
      ...
      <BatchDetail batch={selectedBatch} onClose={() => setSelectedBatch(null)} onComplete={handleComplete} />
    </div>
  );
}
